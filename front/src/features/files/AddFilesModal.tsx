...
export function AddFilesModal({ onClose, id }: AddFilesModalProps) {
  ...
  const onTéléverser = useCallback(() => {
    ...
    if (pwd.length > 0 && pwd.length < MIN_FILE_PASSWORD_LENGTH) {
      setPasswordError(
        `Le mot de passe doit contenir au moins ${MIN_FILE_PASSWORD_LENGTH} caractères.`
      )
      return
    }
  ...
  const uploadError =
    uploadMutation.isError && getApiErrorMessage(uploadMutation.error, "Erreur lors de l'envoi.")
  const shareError =
    shareLinkMutation.isError &&
    getApiErrorMessage(shareLinkMutation.error, 'Erreur lors de la création du lien.')
