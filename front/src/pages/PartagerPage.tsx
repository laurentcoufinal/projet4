...
export function PartagerPage() {
  ...
  const filteredFiles = useMemo(() => {
    ...
    if (tab === 'active') return myFiles.filter((f) => getExpirationStatus(f) === 'active' || getExpirationStatus(f) === 'none')
  ...
  function renderFileListContent() {
    ...
    if (filteredFiles.length === 0) return <p className={styles.emptyState}>{getEmptyMessage(tab)}</p>
    return (
      ...
      {isAuthenticated ? (
        ...
        <div className={styles.fileListCard}>
          <p className={styles.loginPrompt}>
            Connectez-vous pour voir et gérer vos fichiers.
          </p>
        </div>
      )}
    </>
        ...
        {!isMobileView && (
          <aside className={styles.sidebar}>
            ...
            <footer className={styles.sidebarFooter}>
              <span>Copyright DataShare<sup>©</sup> 2025</span>
            </footer>
          </aside>
        )}
        ...
        <div className={styles.contentColumn}>
          {isMobileView ? (
            <>
              ...
              >
                <Link to="/" className={styles.mobileDrawerItem} onClick={() => setMobileMenuOpen(false)}>
                  Accueil
                </Link>
                <span className={`${styles.mobileDrawerItem} ${styles.mobileDrawerItemActive}`} aria-current="page">
                  Mes fichiers
                </span>
                {isAuthenticated && (
                  <>
                    ...
                    <button type="button" className={styles.mobileDrawerItem} onClick={handleLogout}>
                      Déconnexion
                    </button>
                  </>
                )}
                {!isAuthenticated && (
                  <Link to="/connection" className={styles.mobileDrawerItem} onClick={() => setMobileMenuOpen(false)}>
                    Connexion
                  </Link>
                )}
                ...
                <div className={styles.partagerHeaderBarInner}>
                  <div className={styles.partagerHeaderActions}>
                    ...
                    <button type="button" className={styles.btnLogout} onClick={handleLogout}>
                      <svg className={styles.btnLogoutIcon} width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden>
                        <path d="M6 14H3.33333C2.97971 14 2.64057 13.8595 2.39052 13.6095C2.14048 13.3594 2 13.0203 2 12.6667V3.33333C2 2.97971 2.14048 2.64057 2.39052 2.39052C2.64057 2.14048 2.97971 2 3.33333 2H6M10.6667 11.3333L14 8M14 8L10.6667 4.66667M14 8H6" stroke="#E27F29" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                      Déconnexion
                    </button>
                  </div>
